...
read -r rate </sys/class/power_supply/CMB0/power_now
[ "$rate" = "0" ] && notify-send "Battery fully charged" && exit

read -r ac </sys/class/power_supply/ADP1/online
read -r charge_now </sys/class/power_supply/CMB0/energy_now

if [ "$ac" = 1 ] ; then
    read -r charge_full </sys/class/power_supply/CMB0/energy_full
